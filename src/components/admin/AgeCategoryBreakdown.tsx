export function AgeCategoryBreakdown({ data }: { data: { category: string; count: number }[] }) {
  ...
  return (
    <section className="flex h-[280px] flex-col overflow-hidden rounded-[20px] border border-border bg-surface p-4 shadow-lg shadow-secondary/5 sm:p-6">
      ...
      <div
        ...
        tabIndex={0}
        aria-label="Categorías de edad"
      >
        <div className="grid grid-cols-2 gap-3">
          {data.map((row) => (
            <div key={row.category} className="flex min-h-32 flex-col justify-between rounded-[12px] bg-surface-muted p-3.5">
              <div>
