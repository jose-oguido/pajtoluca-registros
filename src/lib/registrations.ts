...
export function createRegistration(data: NewRegistration): { id: number; ticketId: string } {
  const stmt = db.prepare(`
    INSERT INTO registrations
      (ticket_id, full_name, age, phone, email, belongs_to_group, parish_id, parish_group, decanato, zona_pastoral, emergency_contact_name, emergency_contact_phone, notes)
    VALUES
      (@ticket_id, @full_name, @age, @phone, @email, @belongs_to_group, @parish_id, @parish_group, @decanato, @zona_pastoral, @emergency_contact_name, @emergency_contact_phone, @notes)
  `);
...
export function getTodayCount(): number {
  const row = db
    .prepare(`SELECT COUNT(*) as count FROM registrations WHERE date(created_at) = date('now')`)
    .get() as { count: number };
  return row.count;
}

export function getAverageAge(): number {
  const row = db.prepare(`SELECT AVG(age) as avg FROM registrations`).get() as { avg: number | null };
...
export function getTopGroup(): { group: string; count: number } | null {
  const row = db
    .prepare(
      `SELECT parish_group as "group", COUNT(*) as count FROM registrations WHERE belongs_to_group = 1 GROUP BY parish_group ORDER BY count DESC LIMIT 1`
    )
...
export function getGroupBreakdown(): { group: string; count: number }[] {
  return db
    .prepare(
      `SELECT COALESCE(parish_group, '${NO_GROUP_LABEL}') as "group", COUNT(*) as count FROM registrations GROUP BY "group" ORDER BY count DESC`
    )
...
export function getDecanatoBreakdown(): { decanato: string; count: number }[] {
  return db
    .prepare(
      `SELECT COALESCE(decanato, '${NO_GROUP_LABEL}') as decanato, COUNT(*) as count FROM registrations GROUP BY decanato ORDER BY count DESC`
    )
...
export function getAgeCategoryBreakdown(): { category: string; count: number }[] {
  return db
    .prepare(
      `SELECT ${AGE_CATEGORY_CASE} as category, COUNT(*) as count FROM registrations GROUP BY category ORDER BY count DESC`
    )
...
export function listRegistrations(opts: { search?: string; limit?: number; offset?: number }): {
  ...
  const whereClause = search.trim()
    ? `WHERE full_name LIKE @like OR phone LIKE @like OR IFNULL(parish_group, '') LIKE @like OR IFNULL(decanato, '') LIKE @like OR IFNULL(email, '') LIKE @like`
    : "";
